...
public class Game : MonoBehaviour {
	...
	void Update () {
        if (!introFinished) {   // code for cool intro countdown and music
            int t = (int)timer;
            if (t <= 3) {
                if (!songStart) {   // start song up at right time
                    song.time = 19.5f;
                    song.volume = 0f;
                    song.Play();
                    songStart = true;
                ...
                if (t != soundTimer && t > 0) { // noises for the countdown numbers
                    AudioManager.instance.playSound("Collision", Vector3.zero, 1f);
                    soundTimer = t;
                }
                if (t <= 0 && !gameStart) { // game starts here
                ...
                if (t <= 0 && frames > 5) { // flicker text between yellow and white
                    text.color = (text.color == Color.white) ? Color.yellow : Color.white;
                    frames = 0;
                }
                frames++;

                if (t <= -1) {  // intro is over
                    text.text = "";
                    introFinished = true;
                }
            }
